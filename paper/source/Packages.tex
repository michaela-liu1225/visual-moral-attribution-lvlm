% Tectonic uses XeTeX and reads UTF-8 natively; legacy inputenc/fontenc
% declarations are intentionally omitted.
...
% pstricks was inherited from the template but is unused in this thesis.
\usepackage{theorem}
...
\usepackage{esint}
% datetime was inherited from the template but is unused.
\usepackage{caption}
\captionsetup{font=small,labelfont=bf,labelsep=period,skip=6pt}
% smartdiagram was inherited from the template but is unused.
%Set Abstract Page
\usepackage{abstract}
...
% arydshln was inherited from the template but is unused. It is omitted from
% the public build because current array/tabularx releases no longer support
% this legacy combination reliably.
...
\usepackage{hyperref}
%\usepackage{ieeetr}
% lipsum was inherited from the template but is unused.
\usepackage{tikz-cd}
\usepackage{float}
\usepackage{titling}
\usepackage{epigraph}
...
% Algorithm and Gantt packages were inherited from the template but are
% unused by the dissertation.
